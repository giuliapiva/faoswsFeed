...
feedAvail = function(vars, measuredItem = feedNutrients$measuredItemCPC, officialData = NULL, negate = FALSE) {
  ...
  # If no trade variables are requested, all data is from aproduction
  if(nrow(feedCodeTable[variable %in% vars & dataset == "trade"]) != 0) {
    
    tradeData <- getSplitTrade(getQueryKey("geographicAreaM49"), 
                               measuredItem, 
                               feedCodeTable[variable %in% vars & dataset == "trade", measuredElement],
                               feedCodeTable[variable %in% vars & dataset == "trade", measuredElementFS],
                               getQueryKey("timePointYears")
    )
